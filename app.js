...
function colorFor(n){const r=[1,2,7,8,12,13,18,19,23,24,29,30,34,35,40,45,46],b=[3,4,9,10,14,15,20,25,26,31,36,37,41,42,47,48];return r.includes(n)?'red':b.includes(n)?'blue':'green'}function ball(n,s=false){const d=document.createElement('div');d.className=`ball ${colorFor(n)}`;d.textContent=String(n).padStart(2,'0');if(s)d.title='特别号';return d}function setText(s,v){const e=document.querySelector(s);if(e)e.textContent=v}function roi(h,n,name){const o=name==='马'?.8:1.08;return((h*o-(n-h))/n)*100}
function recalc(history){const draws=history.draws.filter(d=>d.issue>='26/047'&&d.issue<='26/999').sort((a,b)=>a.issue.localeCompare(b.issue)),n=draws.length;if(!n)return snapshot;const counts=Object.fromEntries(Array.from({length:49},(_,i)=>[i+1,0]));draws.forEach(d=>d.main.forEach(x=>counts[x]++));const zs=Object.entries(Z).map(([name,nums])=>{let hits=0;draws.forEach(d=>{if(nums.some(x=>[...d.main,d.special].includes(x)))hits++});const recent=draws.slice(-10).filter(d=>nums.some(x=>[...d.main,d.special].includes(x))).length,hitRate=hits/n*100,R=roi(hits,n,name),score=hitRate*.4+Math.max(0,Math.min(100,50+R*2))*.25+recent*10*.1+Math.max(0,100-Math.abs(recent*10-hitRate)*2)*.05;return{name,nums,hits,hitRate,R,recent,score}}).sort((a,b)=>b.score-a.score);const nr=Object.entries(counts).map(([k,v])=>({n:+k,v,label:Object.keys(Z).find(z=>Z[z].includes(+k))})).sort((a,b)=>b.v-a.v).slice(0,10),latest=draws.at(-1);return{...snapshot,checkedAt:history.checkedAt||new Date().toISOString(),latest,sampleCount:n,hotNumber:nr[0].n,hotZodiac:zs[0].name,numberRanking:nr.map((x,i)=>({n:x.n,label:x.label,score:`${x.v}次 / ${n}期`,note:i?'第五代累计':'当前正选频率最高'})),zodiacRanking:zs.map(x=>({name:x.name,nums:x.nums.map(v=>String(v).padStart(2,'0')).join(' · '),note:`命中 ${x.hits}/${n} (${x.hitRate.toFixed(1)}%) · 近10期 ${x.recent}/10 · ROI ${x.R>=0?'+':''}${x.R.toFixed(1)}%`,trend:`评分 ${x.score.toFixed(1)}`})),specialPool:nr.slice(0,6).map(x=>x.n)}}
